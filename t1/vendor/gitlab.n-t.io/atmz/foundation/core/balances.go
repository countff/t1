...
// StateKey constants
const (
	StateKeyNonce StateKey = iota + 42
...
func (bc *BaseContract) tokenBalanceSub(address types.Address, amount *big.Int, token string) error {
	...
	if len(parts) > 1 {
		return balanceSub(bc.stub, StateKeyTokenBalance, address, amount, parts[1])
	}
...
func (bc *BaseContract) tokenBalanceAdd(address types.Address, amount *big.Int, token string) error {
	parts := strings.Split(token, "_")
	if len(parts) > 1 {
		return balanceAdd(bc.stub, StateKeyTokenBalance, address, amount, parts[1])
	}
