...
// Package mocked_atomyze provides APIs for the chaincode to access its state
...
type MockStub struct {
	...
	Invokeables map[string]*MockStub
...
func (stub *MockStub) MockPeerChaincode(invokeableChaincodeName string, otherStub *MockStub) {
	stub.Invokeables[invokeableChaincodeName] = otherStub
}
...
func (stub *MockStub) PutBalanceToState(key string, balance *big.Int) error {
	value := balance.Bytes()
	// If the value is nil or empty, delete the key
	if len(value) == 0 {
		mockLogger.Debug("MockStub", stub.Name, "PutState called, but value is nil or empty. Delete ", key)
...
func (stub *MockStub) InvokeChaincode(chaincodeName string, args [][]byte, channel string) pb.Response {
	...
	otherStub := stub.Invokeables[chaincodeName]
...
func (stub *MockStub) SetCreatorCert(creatorMSP string, creatorCert []byte) error {
...
// GetCreator returns creator. Not implemented
...
func NewMockStub(name string, cc shim.Chaincode) *MockStub {
	...
	s.Invokeables = make(map[string]*MockStub)
	s.Keys = list.New()
